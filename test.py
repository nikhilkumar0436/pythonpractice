...
book1['combined_key'] = book1[file1_columns].astype(str).apply(lambda row: '_'.join(row), axis=1)
book2['combined_key'] = book2[file2_columns].astype(str).apply(lambda row: '_'.join(row), axis=1)
...
for index, row in merged_df.iterrows():
    ...
    if row['amount-of-charge'] == 'multiple-amount':
        if tAmount == row['amount1'] or tAmount == row['amount2'] or tAmount == row['amount3'] or tAmount == row[
            'amount4'] or tAmount == row['amount5'] or tAmount == row['amount6'] or tAmount == row['amount7']:
